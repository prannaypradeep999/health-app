...
/**
 * Hoisted out of the legacy home-meal template so it can be sent as its own
 * system message. OpenAI caches on the longest common *prefix*, so this table
 * only earns a cache hit when identical bytes lead the request; buried behind
 * the per-user schedule it never matched. Worth it here because
 * generateHomeMealsLegacyChunked splits the week into halves and fires two
 * calls back-to-back with this same prefix.
 *
 * Keep this free of ${...} — one per-user token breaks the prefix for everyone.
 *
 * NOTE: these values have drifted from the equivalent table in
 * recipe-creation.ts (e.g. egg carbs). Reconciling them changes nutrition
 * output, so it is deliberately not part of this caching change.
 */
export const HOME_MEAL_NUTRITION_METHOD = `NUTRITION CALCULATION METHOD:
...
5. ⚠️ BUILD THE ORDER, NOT JUST THE ENTRÉE: one plate frequently cannot reach these targets. A meal slot is a real order, and a person filling a large target orders more than one item. When no single menu item lands inside the calorie window AND near the protein target, combine 2-3 items FROM THE SAME RESTAURANT into that one order, the way someone would actually order it — an entrée plus a protein side, an added portion of meat, a soup or salad alongside.
   ...
   - ⚠️ EVERY ITEM IN THE COMBINATION MUST BE A DIFFERENT MENU ITEM. Never order the same item twice to reach a target. "Mighty Kale Salad + Mighty Kale Salad + Mighty Kale Salad" and "Moussaka Vegetarian + Moussaka Vegetarian" are both forbidden, however well the calories add up. Nobody orders three identical salads, and seeing it offered is the point where the user stops believing the plan. Combine an entrée with a DIFFERENT side, soup or salad.
   - If the target is genuinely unreachable without repeating an item, then come in UNDER the calorie target with distinct items and report the real numbers. An honest shortfall is usable; a doubled plate is not. Prefer a different restaurant with a larger or higher-protein single dish before you accept the shortfall.
...
8. ⚠️ VARIETY — RESTAURANTS: No restaurant may be the primary pick for more than ${Math.max(1, Math.ceil(restaurantMealsSchedule.length / Math.max(1, restaurantMenuData.length)))} of the ${restaurantMealsSchedule.length} meals. With ${restaurantMenuData.length} restaurants available, repeating one is a choice, not a necessity.
8b. ⚠️ VARIETY — DISHES: Never select the same dish twice across the week. Two slots at the same restaurant must order different things. This is separate from rule 8 and stricter: staying under the restaurant cap while ordering an identical plate on three different days still reads to the user as one meal repeated, which is the failure this rule exists to prevent. Each restaurant's menu above lists up to 8 items — use them. If a restaurant genuinely has only one dish that fits the targets and the restrictions, pick a different restaurant for the second slot rather than repeating the dish.
8c. ORDERABILITY TIEBREAK: when two restaurants fit the targets and the restrictions equally well, pick the one with a GrubHub ordering link. The candidate list above is already ordered with those first, so preferring an earlier entry is usually right. This is a tiebreak ONLY — never trade nutrition, dietary restrictions or dish variety for it, and a restaurant with no link is still a valid pick when it is the better fit.
